...
class Option(Config):
    ...
    # data_dir = 'Data/pr_10_cross'
    # project = 'train_data_pr'
    # root_dir = os.path.join('Data',project)

    # root dir of testing set
    # test_dir = 'Data/test_data_pr'

    # save segmenting results (prediction masks) to this folder
    # results_dir = 'D:\Data\\validation\\project1'

    num_workers = 1     	# number of threads for data loading
    shuffle = True      	# shuffle the data set
    batch_size = 12     		# GTX1060 3G Memory
    ...
    learning_rate = 0.001 	# learning rage
    weight_decay = 1e-5   	# weight decay

    pin_memory = True   	# use pinned (page-locked) memory. when using CUDA, set to True
    # save segmenting results (prediction masks) to this folder
    # projectpath = './results'
    # results_dir = os.path.join(projectpath, str(epochs) + "-" +str(batch_size)  + "-" + name)
...
class Utils(object):
    ...
    def prepare_training_data(self):
        ...
        for n, id_ in tqdm(enumerate(train_ids)):
            ...
            img = Image.open(os.path.join(path, 'images', id_ + '.bmp')).convert("RGB")
            mask = self.assemble_masks(path)
            img.save(os.path.join(dest, 'image.bmp'))
            Image.fromarray(mask).save(os.path.join(dest, 'mask.bmp'))
    ...
    def prepare_testing_data(self):
        ...
        for n, id_ in tqdm(enumerate(test_ids)):
            path = os.path.join(self.stage1_test_src, id_, 'images', id_+'.bmp')
...
def compute_iou(filename,predictions, img_ids, val_loader,test_path,savepath):
    """
    ...
    for i in range(0, len(img_ids)):
        pred = predictions[i]
        img_id = img_ids[i]
        mask_path = os.path.join(test_path, img_id, 'mask.bmp')
        ...
        ious.append(iou)
        dscs.append(dsc)
    df = pd.DataFrame({'img_id':img_ids,'iou':ious,'dsc':dscs})
    csvpath = os.path.join(savepath,str(filename)+'-'+'DSC.csv')
...
def encode_and_save(preds_test_upsampled, test_ids,savepath):
    """
    ...
    for i in range(0, len(test_ids)):
        path = os.path.join(savepath, test_ids[i])
