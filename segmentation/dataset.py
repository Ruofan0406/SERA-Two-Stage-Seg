...
class DSB2018Dataset(Dataset):
    ...
    def __getitem__(self, idx):
        if self.train:
            img_dir = os.path.join(self.root_dir, self.img_id[idx], 'image.bmp')
            mask_dir = os.path.join(self.root_dir, self.img_id[idx], 'mask.bmp')
            img = io.imread(img_dir).astype(np.uint8)
            mask = io.imread(mask_dir, as_gray=True).astype(np.bool)
...
def get_train_valid_loader(root_dir, batch_size=16, split=True,
    ...
    if split:
        ...
        val_transformed_dataset = DSB2018Dataset(root_dir=root_dir,
                                                 ...
                                                 train=True,
                                                 transform=transforms.Compose([
                                                      # RandomCrop(128),    # for validation set, do not use augmentation
                                                      # Rescale(128),
                                                      ToTensor()
                                                   ]))
...
def get_test_loader(root_dir, batch_size=1, shuffle=False, num_workers=4, pin_memory=False):
